...
public class Jogo {
	...
	public void rodada() throws EstrategiaInvalidaException, JogadaInvalidaException {
		...
		if (jogador1.getNumPecas() == 0) {
			...
			this.pontuacao = calculaPontuacao(jogador1);
			return;
		}
		...
		if (jogador2.getNumPecas() == 0) {
			...
			this.pontuacao = calculaPontuacao(jogador2);
			return;
		}

		// se ambos passaram checa-se as m�o, caso tenham mesmo n�mero de pecas e mesma soma, � um empate.
		if (jogadaJ1.getTipo() == TipoJogada.PASSA && jogadaJ2.getTipo() == TipoJogada.PASSA) {
			if(jogador1.getNumPecas() != jogador2.getNumPecas()) {
				this.vencedor = (jogador1.getNumPecas() < jogador2.getNumPecas()) ? this.jogador1.getNome() : this.jogador2.getNome();
				this.pontuacao = 1;
			} 
			else if(jogador1.getSomaMao() != jogador2.getSomaMao()) {
				this.vencedor = (jogador1.getSomaMao() < jogador2.getSomaMao()) ? this.jogador1.getNome() : this.jogador2.getNome();
				this.pontuacao = 1;
			}
			else {
				this.vencedor = null;
	...
	private int calculaPontuacao(Jogador vencedor) {
		boolean laElo = false;
		boolean carroca = false;
		
		if(vencedor.getUltimaJogada().getTipo() == TipoJogada.NA_DIREITA) {
			if(vencedor.getUltimaJogada().getPeca().encaixa(this.mesa.getNumNaEsquerda())) 
				laElo = true;
		} else if(vencedor.getUltimaJogada().getTipo() == TipoJogada.NA_ESQUERDA) {
			if(vencedor.getUltimaJogada().getPeca().encaixa(this.mesa.getNumNaDireita())) 
				laElo = true;
		}
		
		if(vencedor.getUltimaJogada().getPeca().getNumDireito() == vencedor.getUltimaJogada().getPeca().getNumEsquerdo()) {
			carroca = true;
		}
	...
	public HistoricoDeJogo jogaJogoCompleto() throws EstrategiaInvalidaException, JogadaInvalidaException {
		...
		if(this.isResultadoEmpate()) {
			...
			jogado.setVencedor(getVencedor(), getPontuacao());
		}
...
//		System.out.println("==> final: " + (venceu == null ? "EMPATE" : venceu + " VENCEU") + "\n");
	...
	/**
	 * @return Pontua��o ganha no jogo. 0 no caso de empate ou o jogo n�o tenha acabado.
	 */
